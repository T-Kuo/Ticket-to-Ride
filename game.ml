...
let rec execute_turn state action num =
  match num with
  |0 -> (
    match action with
    | DrawFaceUp n ->
      ...
      |Rainbow ->
        ...
        let nfu = dr::(List.tl state.train_deck.faceup)
        let nd = {state.train_deck with
                  draw_pile = List.tl state.train_deck.draw_pile;
                  faceup = nfu} in
        let np = {pl with cd::pl.train_hand} in
        let ns = {state with
    ...
    | ClaimRoute (rt,c) ->
      ...
      |true, bd ->
        let np = {pl with
                  ...
                  score = (calculate_score pl.pid bd.routes 0)}
    ...
    | RequestTickets ->
      ...
      let np = {pl with cd::pl.ticket_hand} in
      let ns = {state with
        player_info = (List.tl state.player_info) @ [np]
        ticket_deck = nd} in
      do_turn ns
...
let do_turn2 state =
  ...
  match pl.ptype with
  |Human ->
    let a = (Gui.do_turn state.board pl.player pl.ticket_hand
